...
func (r *OrderPostgres) GetAll(userId int) ([]backend.Order, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.category, tl. status, tl.date, tl.comment FROM %s tl INNER JOIN %s ul on tl.id = ul.order_id WHERE ul.user_id = $1",
...
func (r *OrderPostgres) GetById(userId, orderId int) (backend.Order, error) {
	...
	query := fmt.Sprintf(`SELECT tl.id, tl.category, tl. status, tl.date, tl.comment FROM %s tl
